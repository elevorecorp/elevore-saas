...
-- 2. Entra a tu proyecto, ve a la sección "SQL Editor".
-- 3. Crea una nueva consulta ("New Query"), pega este código completo y haz clic en "RUN".
-- =====================================================================

-- ---------------------------------------------------------
-- 1. Tabla de Tenants (Inquilinos/Negocios Registrados)
-- ---------------------------------------------------------
...
COMMENT ON TABLE public.tenants IS 'Almacena la información de los negocios que se registran en el SaaS.';
...
CREATE TABLE IF NOT EXISTS public.tenant_settings (
    ...
    admin_pin TEXT DEFAULT '2026', -- PIN personalizado para vista Admin rápida de cada negocio
    
    -- Tarifas base y catálogos en JSONB para máxima flexibilidad
...
-- ---------------------------------------------------------
-- 3. Tabla de Staff Profiles (Asignación de empleados a Negocios)
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 4. Modificación de Tablas Existentes: Clientes y Misiones
-- ---------------------------------------------------------

-- Agregar columna tenant_id a public.clients si no existe
DO $$
BEGIN
...
-- Agregar columna tenant_id a public.elevore_missions si no existe
DO $$
BEGIN
...
-- ---------------------------------------------------------
-- 5. Row Level Security (RLS) - Aislamiento Absoluto de Datos
-- ---------------------------------------------------------
...
-- Crear políticas para Tenant Settings
...
-- Crear políticas para Staff Profiles
...
-- Crear políticas para Elevore Missions (Trabajos/Misiones)
...
-- Permitir a clientes sin autenticación ver su propio portal de misión vía el ID de la misión (token UUID)
CREATE POLICY "Public anonymous clients can access their specific mission portal" ON public.elevore_missions
    FOR SELECT USING (true); -- Cualquiera con el UUID de la URL puede ver el portal de firma

CREATE POLICY "Public anonymous clients can sign and rate their mission" ON public.elevore_missions
    FOR UPDATE USING (true); -- Permitir firmas digitales y calificaciones finales anónimas

-- ---------------------------------------------------------
-- 6. Trigger Automático para Crear Configuración al Registrar un Tenant
-- ---------------------------------------------------------
